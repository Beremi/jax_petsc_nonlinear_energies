...
\subsection{Plasticity3D source and sourcefixed comparisons}
...
source-assembly comparator and a sourcefixed-like PMG variant.
...
\begin{table}[H]
  \centering
  \input{tables/generated/plasticity3d_sourcefixed_alternative.tex}
  \caption{Converged-only view of the sourcefixed-like PMG alternative on the
  same Plasticity3D problem. ``--'' marks rank counts that hit the Newton cap
  before reaching the promoted gradient target.}
  \label{tab:plasticity3d-sourcefixed}
\end{table}
...
the sourcefixed-like PMG profile is not the preferred maintained default in the
