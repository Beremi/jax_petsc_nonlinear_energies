...
\paragraph{Constitutive AD is the best derivative route on the flagship
Plasticity3D case.}
...
\code{P4(L1)}, $\lambda_{\mathrm{sr}}=1.5$, 8-rank case. All three routes
...
The new Plasticity3D validation ladder in
...
result for software credibility. However, the fixed-lambda source-operator
comparison does not satisfy the stricter agreement thresholds on
$u_{\max}(\lambda)$ and endpoint displacement fields. The right interpretation
is therefore limited but still publishable: the maintained implementation
faithfully reproduces the intended surrogate workflow, yet the present evidence
does not justify describing that surrogate as constitutively equivalent to a
...
scrutiny. Here the exact mesh, constitutive law, and displacement schedule are
matched, and the resulting energy and field differences are very small. That
baseline is useful precisely because it is narrow. The paper should not
generalize it into a broad ranking between frameworks, but it does strengthen
the manuscript by showing that at least one recent external implementation can
be matched on a shared problem contract.
...
scaling campaign at $\lambda_{\mathrm{sr}}=1.0$ is historical workflow
