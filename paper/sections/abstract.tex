...
passes the configured gated quantities, with $u_{\max}(\lambda)$ relative
$L^2$ difference \num{6.396e-6} and endpoint displacement relative $L^2$
difference \num{8.299e-4}; endpoint deviatoric-strain and boundary-profile
agreement are reported as diagnostics. This validates the implemented endpoint
surrogate within the tested fixed-load contract, but not as a path-consistent
replacement for an incremental elastoplastic history solve. On the locked
$P_4(L_1)$,
