...
prescribed displacement schedule, with repository-energy post-comparison; it is
included as a fairness-first hyperelastic implementation comparison rather than
...
\begin{figure}[H]
  ...
  curated $P_4(L_5)$ showcase and on the larger fixed-work $P_4(L_6)$ and
  $P_4(L_7)$ diagnostics, which are not used as endpoint-convergence evidence.}
...
paper. Only the curated $P_4(L_5)$ row is treated as a completed endpoint
solve; the larger $P_4(L_6)$ and $P_4(L_7)$ rows are fixed-work diagnostics for
solver behavior. Solver-engineering evidence for Plasticity2D is reported
later in \Cref{sec:num-plasticity2d,sec:solver-evidence}.
...
with $|y| \le 10^{-12}$ in all three displacement components. We write
$P_k(L_\ell)$ for polynomial degree $k$ on mesh level $\ell$; the repository
mesh identifiers \code{L1}, \code{L1\_2}, \code{L1\_2\_3}, and
\code{L1\_2\_3\_4} are displayed in the manuscript as $L_1,\ldots,L_4$.
The figures below use the corrected glued-bottom $P_4(L_2)$ showcase at
$\lambda_{\mathrm{sr}}=1.55$ and the finished nine-case maintained study
at the same load factor. The promoted $\lambda_{\mathrm{sr}}=1.0$ strong-scaling
study is kept later in the paper as separate historical timing evidence.
...
materials, Davis-B reduction parameters, load schedule, and glued-bottom
free-DOF mask, but it is not a path-consistent incremental-history validation.
The paper therefore keeps the Plasticity3D claims at the level of a
repository-specific endpoint surrogate rather than a path-consistent
replacement. The related
return-mapping, optimization, convex-optimization, and continuation literature
\citep{sysala2017returnmapping,sysala2021optimization,sysala2025convexoptimization,sysala2025advancedcontinuation} is used as
...
\begin{figure}[H]
  ...
  strain for the maintained $P_4(L_2)$ run, both derived from the
  scalar surrogate in
...
\begin{figure}[H]
  ...
  glued-bottom maintained Plasticity3D study runs at
...
\begin{table}[H]
  ...
  \caption{All nine completed glued-bottom maintained Plasticity3D
...
\begin{figure}[H]
  ...
  runs use the corrected glued-bottom maintained solver stack at
