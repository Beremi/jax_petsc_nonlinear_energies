...
The strongest new evidence is now more precise than the paper's original broad
claim. On Plasticity3D, same-case source-faithfulness to the source-style
surrogate implementation is strong, while the fixed-lambda source-operator
comparison does not support constitutive-equivalence language. The correct
reading is therefore that the repository provides a supported maintained
surrogate workflow for this benchmark family, with constitutive AD emerging as
the best cost-quality route on the locked flagship case. On Hyperelasticity,
...
\begin{itemize}
  \item extend the validation surface for path-dependent mechanics beyond the
        current surrogate-versus-source-operator comparison, including a true
...
The next scientific step is not to overclaim the current implementation, but
