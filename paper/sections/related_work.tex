...
The literature most relevant to this manuscript falls into six closely connected
groups: high-level finite-element automation, PDE-level adjoint optimization,
JAX-native differentiable FEM, JAX-to-FEM bridge mechanisms, topology
optimization workflows, and sparse nonlinear solver infrastructure. The purpose
of this section is not to assign a single ranking to those groups, but to state
precisely which part of each literature thread the present paper uses as context.
...
such workflows in practice \citep{mitusch2019pyadjoint}. The current
...
\citep{xue2023jaxfem}. AutoPDEx broadens that JAX-native PDE picture with
nonlinear minimizers, implicit differentiation, and optional PETSc integration
\citep{bode2025autopdex}. JAX-CPFEM is especially relevant to the mechanics side
of the present paper because it couples differentiable FEM, nonlinear crystal
plasticity, and GPU execution in a recent open-access workflow
\citep{hu2025jaxcpfem}. Together, these papers define the closest modern
literature context for the repository's JAX-based local differentiation layer.
...
story into a PETSc-centered sparse
assembly and nonlinear-solve setting with explicit comparison between element AD,
constitutive AD, and colored SFD.
